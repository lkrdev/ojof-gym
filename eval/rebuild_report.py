...
def rebuild_run_report(
    ...
    update_jetski_artifact: bool = True
):
    ...
    for task_data in master_results.get("tasks", []):
        ...
        report_file = task_dir / "eval_report.md"
        report_file.write_text(md_report)
        print(f"  [OK] Saved updated report: {report_file}")

        if update_jetski_artifact:
            artifact_dir = os.environ.get("JETSKI_ARTIFACT_DIR")
            if artifact_dir and Path(artifact_dir).exists():
                art_file = Path(artifact_dir) / f"{scenario_name}_evaluation_report.md"
                art_file.write_text(md_report)
                print(f"  [OK] Synced to Jetski Artifact: {art_file}")
            else:
                ...
                if app_data and conv_id:
                    ...
                    if p.parent.exists():
                        ...
                        print(f"  [OK] Synced to Jetski Artifact: {p}")
