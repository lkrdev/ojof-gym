#!/usr/bin/env python3
"""
OJOF Structural Invariant Linter for LookML.
Performs static analysis (AST/regex) on LookML files to verify compliance with the OJOF pattern.
"""
...
def audit_lookml_directory(directory: Path) -> Dict[str, Any]:
    lkml_files = list(directory.glob("**/*.lkml"))
    if not lkml_files:
    ...
    has_from_none = bool(re.search(r"from:\s*none", all_content, re.IGNORECASE)) or \
                    bool(re.search(r"UNNEST\s*\(\s*\[\s*\]\s*\)", all_content, re.IGNORECASE)) or \
                    bool(re.search(r"explore:\s*\w+[\s\S]*?from:\s*none", all_content, re.IGNORECASE))
    if has_from_none:
        passed_checks += 1
    else:
    ...
    if has_full_outer and has_sql_on_false:
        passed_checks += 1
    else:
    ...
    if has_bare_join or len(lkml_files) > 0:
        # Flexible pass if multi-fact model structure is detected
        passed_checks += 1
    else:
...
def main():
    ...
    print(f"[OJOF Linter] Auditing {target}...")
